"""Routines to perform gridding of (non-Cartesian) under-sampled data with simultaneous low-rank subspace projection."""
import numba as nb
import torch
...
def _nonuniform2uniform_lowrank_cpu_1d(
...
@nb.njit(fastmath=True, parallel=True, cache=True)  # pragma: no cover
def _nonuniform2uniform_lowrank_cpu_2d(
    cart_data, noncart_data, interp_value, interp_index, basis
):
    # get sizes
    ncoeff, batch_size, _, _ = cart_data.shape
...
@nb.njit(fastmath=True, parallel=True, cache=True)  # pragma: no cover
def _nonuniform2uniform_lowrank_cpu_3d(
    cart_data, noncart_data, interp_value, interp_index, basis
):
    # get sizes
...
# function table
_nonuniform2uniform_lowrank_cpu = [_nonuniform2uniform_lowrank_cpu_1d, _nonuniform2uniform_lowrank_cpu_2d, _nonuniform2uniform_lowrank_cpu_3d]

#%% GPU
if torch.cuda.is_available():

    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_1d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _ = cart_data.shape
    ...
    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_2d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _ = cart_data.shape
    ...
    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_3d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _, _ = cart_data.shape
    ...
    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_cplx_1d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _ = cart_data.shape
    ...
    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_cplx_2d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _ = cart_data.shape
    ...
    @nb.cuda.jit()
    def _nonuniform2uniform_lowrank_gpu_cplx_3d(
        cart_data, noncart_data, interp_value, interp_index, basis
    ):
        # get sizes
        ncoeff, batch_size, _, _, _ = cart_data.shape
    ...
    # function table
    _nonuniform2uniform_lowrank_gpu = [_nonuniform2uniform_lowrank_gpu_1d, _nonuniform2uniform_lowrank_gpu_2d, _nonuniform2uniform_lowrank_gpu_3d]
    _nonuniform2uniform_lowrank_gpu_cplx = [_nonuniform2uniform_lowrank_gpu_cplx_1d, _nonuniform2uniform_lowrank_gpu_cplx_2d, _nonuniform2uniform_lowrank_gpu_cplx_3d]
