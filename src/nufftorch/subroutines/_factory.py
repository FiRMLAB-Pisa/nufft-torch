...
class NUFFTFactory(AbstractFactory):
    ...
    def _prepare_kernel(self, coord, shape, width, beta, basis, device):
        ...
        if basis is not None:
            ...
            basis_adjoint = _subroutines.BackendBridge.pytorch2numba(
                basis_adjoint.to(device)
            )
        else:
            basis_adjoint = None
    ...
    def _prepare_sparse_coefficient_matrix(self, coord, shape, width, beta, device):
        ...
        # actual computation
        for i in range(self.ndim):
            val, ind, coo = _subroutines.BackendBridge.pytorch2numba(
...
class NonCartesianToeplitzFactory(AbstractFactory):
    ...
    def _initialize_mtf(
        ...
        return (
            _subroutines.Grid(interpolator["device_dict"])(
                dcf, interpolator["kernel_dict"]
            )
            * scale
        )
