...
from nufftorch.subroutines import _cpu
...
class NUFFTFactory(AbstractFactory):
    ...
    def __call__(self, coord, shape, width, oversamp, basis, device, threadsperblock):
        ...
        # get arrays
        width, shape = _subroutines.Utils._scalars2arrays(ndim, width, shape)

        # get kernel shape parameter
        beta = _subroutines.Utils._beatty_parameter(width, oversamp)

        # get grid shape
        grid_shape = _subroutines.Utils._get_oversampled_shape(oversamp, shape)

        # adjust coordinates
        coord = _subroutines.Utils._scale_coord(coord, shape, oversamp)

        # compute interpolator
        kernel_dict = self._prepare_kernel(coord, grid_shape, width, beta, basis, device)
        
        # get scaling
        scale = _subroutines.Utils._get_kernel_scaling(beta, width)
    ...
    def _prepare_kernel(self, coord, shape, width, beta, basis, device):
        ...
        if basis is not None:
            ...
            basis = _subroutines.BackendBridge.pytorch2numba(basis.to(device))
            basis_adjoint = _subroutines.BackendBridge.pytorch2numba(basis_adjoint.to(device))
        else:
            basis_adjoint = None
    ...
    def _reformat_sparse_coefficients(self, value, index, width, device):
        ...
        value, index = _subroutines.BackendBridge.pytorch2numba(value, index)
    ...
    def _prepare_sparse_coefficient_matrix(self, coord, shape, width, beta, device):
        ...
        # actual computation
        for i in range(self.ndim):
            val, ind, coo = _subroutines.BackendBridge.pytorch2numba(value[i], index[i], coord[i])
        ...
        offset, width = _subroutines.BackendBridge.pytorch2numba(offset, width)
...
class NonCartesianToeplitzFactory(AbstractFactory):
    ...
    def _initialize_mtf(self, coord, shape, width, prep_osf, basis, device, threadsperblock, dcf):
        ...
        return _subroutines.Grid(interpolator['device_dict'])(dcf, interpolator['kernel_dict'])  * scale

    def _resample_mtf(self, mtf, shape, prep_osf, comp_osf, width, device):
        
        # get arrays
        width, shape = _subroutines.Utils._scalars2arrays(self.ndim, width, shape)

        # get kernel shape parameter
        beta = _subroutines.Utils._beatty_parameter(width, prep_osf)

        # IFFT
        psf = _subroutines.IFFT(mtf)(mtf, axes=range(-self.ndim, 0), norm=None)
        
        # Crop
        psf = _subroutines.Crop(shape[-self.ndim:])(psf)

        # Apodize
        _subroutines.Apodize(shape[-self.ndim:], prep_osf, width, beta, device)(psf)

        # Zero-pad
        psf = _subroutines.ZeroPad(comp_osf, shape[-self.ndim:])(psf)

        # FFT
        mtf = _subroutines.FFT(psf)(psf, axes=range(-self.ndim, 0), norm=None)
    ...
    def _post_process_mtf(self, mtf):
        ...
        # transform to numba
        if self.islowrank:
            mtf = _subroutines.BackendBridge.pytorch2numba(mtf)
