...
def nufft(image: Tensor, interpolator: Dict) -> Tensor:
    ...
    dispatcher = _subroutines.DeviceDispatch(computation_device=device, data_device=image.device)
    image = dispatcher.dispatch(image)

    # Apodize
    _subroutines.Apodize(image.shape[-ndim:], oversamp, width, beta, device)(image)

    # Zero-pad
    image = _subroutines.ZeroPad(oversamp, image.shape[-ndim:])(image)

    # FFT
    kdata = _subroutines.FFT(image)(image, axes=range(-ndim, 0), norm=None)

    # Interpolate
    kdata = _subroutines.DeGrid(device_dict)(kdata, kernel_dict) / scale
...
def nufft_adjoint(kdata: Tensor, interpolator: Dict) -> Tensor:
    ...
    # Offload to computational device
    dispatcher = _subroutines.DeviceDispatch(computation_device=device, data_device=kdata.device)
    kdata = dispatcher.dispatch(kdata)

    # Gridding
    kdata = _subroutines.Grid(device_dict)(kdata, kernel_dict) / scale

    # IFFT
    image = _subroutines.IFFT(kdata)(kdata, axes=range(-ndim, 0), norm=None)

    # Crop
    image = _subroutines.Crop(shape[-ndim:])(image)

    # Apodize
    _subroutines.Apodize(shape[-ndim:], oversamp, width, beta, device)(image)
...
def toeplitz_convolution(image: Tensor, toeplitz_dict: Dict) -> Tensor:
    ...
    # Zero-pad
    image = _subroutines.ZeroPad(oversamp, ishape[-ndim:])(image)

    # FFT
    kdata_in = _subroutines.FFT(image)(image, axes=range(-ndim, 0), norm=None, center=False)
    ...
    if islowrank:
        ...
        _subroutines.Toeplitz(kdata_in.numel(), device_dict)(kdata_out, kdata_in, mtf)
        kdata_out = kdata_out.T.reshape(os_shape).contiguous()
    else:
        kdata_out = mtf * kdata_in

    # IFFT
    image = _subroutines.IFFT(kdata_out)(kdata_out, axes=range(-ndim, 0), norm=None, center=False)

    # Crop
    image = _subroutines.Crop(ishape[-ndim:])(image)
