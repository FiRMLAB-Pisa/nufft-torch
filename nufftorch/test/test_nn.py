...
def test_nufft_selfadjoint(ndim, nframes, nechoes, ncoils, nslices, device, 
    ...
    # define tolerance
    tol = 5e-2  # toeplitz is only approximate
    ...
    # normalize
    # img_ground_truth = _utils.normalize(img_ground_truth, ndim)
    # img_toeplitz = _utils.normalize(img_toeplitz, ndim)
...
# @pytest.mark.parametrize("ndim, nframes, nechoes, ncoils, nslices, dtype, device",
#     list(itertools.product(*[ndim, nframes, nechoes, ncoils, nslices, dtype, device])))
# def test_nufft_selfadjoint_lowrank(ndim, nframes, nechoes, ncoils, nslices, dtype, device, npix=16, test=True):
